param(
    ...
    [string]$Action = "bit",
    [ValidateSet("soc", "soc_sysclk_ila", "soc_bootdiag_sysclk_ila", "hello_sysclk_ila", "cnn_sysclk_ila", "bootvec_sysclk_ila", "heartbeat", "heartbeat_direct", "heartbeat_mmcm_ledonly", "heartbeat_mmcm_dualclk", "heartbeat_mmcm_sysclk_ila")]
...
if ($BuildMode -ne "soc") {
    $topNames = @{
        ...
        cnn_sysclk_ila = "cnn_sysclk_ila_system.v"
        soc_bootdiag_sysclk_ila = "soc_bootdiag_sysclk_ila_system.v"
        hello_sysclk_ila = "hello_sysclk_ila_system.v"
        bootvec_sysclk_ila = "bootvec_sysclk_ila_system.v"
...
if ($BuildMode -eq "soc_bootdiag_sysclk_ila") {
    $extraXdc = Join-Path $boardDir "script\soc_bootdiag_sysclk_ila.xdc"
    if (-not (Test-Path $extraXdc)) {
        throw "SoC boot diagnostic sysclk ILA XDC not found: $extraXdc"
    }
    $env:EXTRA_XDCS = (& $normalize $extraXdc)
}

if ($BuildMode -eq "cnn_sysclk_ila") {
    $extraXdc = Join-Path $boardDir "script\cnn_sysclk_ila.xdc"
    if (-not (Test-Path $extraXdc)) {
        throw "CNN sysclk ILA XDC not found: $extraXdc"
    }
    $env:EXTRA_XDCS = (& $normalize $extraXdc)
}

if ($BuildMode -eq "bootvec_sysclk_ila") {
    $extraXdc = Join-Path $boardDir "script\bootvec_sysclk_ila.xdc"
    if (-not (Test-Path $extraXdc)) {
        throw "Boot vector sysclk ILA XDC not found: $extraXdc"
    }
    $env:EXTRA_XDCS = (& $normalize $extraXdc)
}

if ($BuildMode -eq "hello_sysclk_ila") {
    $extraXdc = Join-Path $boardDir "script\hello_sysclk_ila.xdc"
    if (-not (Test-Path $extraXdc)) {
        throw "Hello sysclk ILA XDC not found: $extraXdc"
    }
    $env:EXTRA_XDCS = (& $normalize $extraXdc)
}
