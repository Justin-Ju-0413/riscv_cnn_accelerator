...
$srcDir = Join-Path $repoRoot "sw\hello_e203"
...
$elf = Join-Path $buildDir "hello_e203.elf"
$verilog = Join-Path $buildDir "hello_e203.verilog"
$dump = Join-Path $buildDir "hello_e203.dump"
$map = Join-Path $buildDir "hello_e203.map"
...
# Convert byte-level hex to 64-bit word-level for Vivado/iverilog compatibility
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $prepare -SocDir $SocDir -Board $Board -AppDir $buildDir -AppName "hello_e203" -ExtraDefines "E203_FORCE_BOOTROM_BOOT"

Write-Host "HELLO_E203_ELF=$elf"
Write-Host "HELLO_E203_VERILOG=$verilog"
Write-Host "HELLO_E203_ITCM=$($verilog -replace '\.verilog$', '.itcm.verilog')"
Write-Host "HELLO_E203_DTCM=$($verilog -replace '\.verilog$', '.dtcm.verilog')"
